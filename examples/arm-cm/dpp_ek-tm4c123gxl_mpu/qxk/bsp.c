...
/* Test1 thread.............................................................*/
enum { TEST1_SIZE_POW2 = 9 };
__attribute__((aligned((1U << TEST1_SIZE_POW2))))
uint8_t Test1_sto[1U << TEST1_SIZE_POW2];

void * const XT_Test1_stackSto = &Test1_sto[0];
uint_fast16_t const XT_Test1_stackSize =
    (1U << TEST1_SIZE_POW2) - sizeof(QXThread);
QXThread * const XT_Test1 =
    (QXThread *)&Test1_sto[(1U << TEST1_SIZE_POW2) - sizeof(QXThread)];

#ifdef QF_MEM_ISOLATE
static MPU_Region const MPU_Test1[3] = {
    { (uint32_t)Test1_sto + 0x10U,             /*---- region #0 */
      ((TEST1_SIZE_POW2 - 1U) << MPU_RASR_SIZE_Pos)   /* size */
       + (3U << MPU_RASR_AP_Pos)                      /* PA:rw/UA:rw */
       + (1U << MPU_RASR_XN_Pos)                      /* XN=1 */
       + (1U << MPU_RASR_S_Pos)                       /* S=1 */
       + (1U << MPU_RASR_C_Pos)                       /* C=1 */
...
/* Test2 thread.............................................................*/
enum { TEST2_SIZE_POW2 = 9 };
__attribute__((aligned((1U << TEST2_SIZE_POW2))))
uint8_t Test2_sto[1U << TEST2_SIZE_POW2];

void * const XT_Test2_stackSto = &Test2_sto[0];
uint_fast16_t const XT_Test2_stackSize =
    (1U << TEST2_SIZE_POW2) - sizeof(QXThread);
QXThread * const XT_Test2 =
    (QXThread *)&Test2_sto[(1U << TEST2_SIZE_POW2) - sizeof(QXThread)];

#ifdef QF_MEM_ISOLATE
static MPU_Region const MPU_Test2[3] = {
    { (uint32_t)Test2_sto + 0x10U,             /*---- region #0 */
      ((TEST2_SIZE_POW2 - 1U) << MPU_RASR_SIZE_Pos)   /* size */
       + (3U << MPU_RASR_AP_Pos)                      /* PA:rw/UA:rw */
       + (1U << MPU_RASR_XN_Pos)                      /* XN=1 */
       + (1U << MPU_RASR_S_Pos)                       /* S=1 */
       + (1U << MPU_RASR_C_Pos)                       /* C=1 */
...
void BSP_init(void) {
    ...
    *(uint32_t volatile *)&GPIOF_AHB->CR = 0x00U;
    ...
    Test1_ctor(XT_Test1, MPU_Test1);
    Test2_ctor(XT_Test2, MPU_Test2);
#else
    Table_ctor(AO_Table, (void *)0);
    Test1_ctor(XT_Test1, (void *)0);
    Test2_ctor(XT_Test2, (void *)0);
#endif
}
/*..........................................................................*/
void BSP_ledOn(void) {
