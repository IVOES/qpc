...
uint16_t QS_rxGetNfree(void) {
    ...
    if (head == QS_rxPriv_.tail) { /* buffer empty? */
        ...
        nFree = (uint16_t)(QS_rxPriv_.end + QS_rxPriv_.tail - (head + 1U));
    }
    return nFree;
}
    ...
    switch (state) {
        ...
        case WAIT4_FILTER_FRAME: {
            ...
            if (l_rx.var.flt.recId == (int8_t)QS_RX_GLB_FILTER) {
                for (i = 0U; i < Q_DIM(QS_filter_.glb); ++i) {
                    QS_filter_.glb[i] = l_rx.var.flt.data[i];
                }
                ...
                */
                ...
                for (i = 0U; i < Q_DIM(QS_filter_.loc); ++i) {
                    QS_filter_.loc[i] = l_rx.var.flt.data[i];
                }
                /* leave QS_ID == 0 always on */
                QS_filter_.loc[0] |= 0x01U;
            }
            else {
...
bool QS_RX_PUT(uint8_t const b) {
    ...
    if (head != QS_rxPriv_.tail) { /* buffer NOT full? */
        ...
        status = true;  /* byte placed in the buffer */
    }
    else {
        status = false; /* byte NOT placed in the buffer */
    }
    return status;
}
/*$enddef${QS::QS-rx} ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
